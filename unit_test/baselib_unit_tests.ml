...
let trivial () = check string "I'm trivial" "hello" "hello"
...
let seq_iteri n msgs () =
  ...
  let f i x = an_array.(i) <- mk_msg i x in
...
let seq_iteri_0 = seq_iteri 3 [| "Hello 0.0!"; "Hello 1.1!"; "Hello 2.2!" |]
  ...
  let case str n (p1, p2) =
    ...
    test_case (sprintf "string_partition_%d" id) `Quick (fun () ->
        check
          ...
          (sprintf "string_partition_opt.case %S %d (%S, %S)" str n p1 p2)
          (Some (p1, p2))
          (String.partition_opt n str);
        check (pair string string)
          (sprintf "string_partition_exn.case %S %d (%S, %S)" str n p1 p2)
          (p1, p2) (String.partition n str))
  in
  let case_fail str n =
    (* failure cases *)
    let id = get_and_decr counter |> ( ~- ) in
    test_case (sprintf "string_partition_%d" id) `Quick (fun () ->
        check
          (option (pair string string))
          (sprintf "string_partition_opt.should_fail %S %d" str n)
          None
          (String.partition_opt n str);
        try
          let p1, p2 = String.partition n str in
          failf "string_partition_exn.should_fail %S %d but got (%S, %S)" str n
            p1 p2
...
let stream_drop_1 =
  stream_drop (list string) 0 [ "A"; "B"; "C" ] [ "A"; "B"; "C" ]
...
let base64_known () =
  ...
  in
  cases
  |> List.iter (fun (plain, code, (module B64 : Base64.T)) ->
         let expected_plain = Bytes.of_string plain in
         let expected_code = code in
         let actual_code = B64.encode expected_plain in
         check string
           (sprintf "base64: encode '%s' => %s" plain expected_code)
           expected_code actual_code;
         let actual_plain = B64.decode expected_code in
         check bytes
           (sprintf "base64: decode '%s' => %s" code (String.escaped plain))
           expected_plain actual_plain)
...
let base64_range () =
  ...
  in
  input_cases
  |> List.iter (fun (offset, len, expected_code) ->
         let actual_code = Base64.encode ~offset ~len orig in
         check string
           (sprintf "base64: encode with range (offset=%d, len=%d) => '%s'"
              offset len expected_code)
           expected_code actual_code);
  let output_cases =
    [
  ...
  in
  output_cases
  |> List.iter (fun (code, offset, len, expected_plain) ->
         let actual_plain = Base64.decode ~offset ?len code in
         check bytes
           (sprintf "base64: decode '%s' with range (offset=%d, len=%s) => '%s'"
              code offset
              (match len with None -> "None" | Some d -> sprintf "%d" d)
              expected_plain)
           (Bytes.of_string expected_plain)
           actual_plain)

let base64_decode_pad () =
  let wrong_padding =
    Either.left (Invalid_argument "Base64.decode: wrong padding")
  in
  let wrong_padding_len len =
    Either.left (Base64.Invalid_base64_padding (`invalid_padding_length len))
  in
  ...
  in
  input_cases
  |> List.iter (fun (input_case, expected) ->
         match expected with
         ...
         | Either.Left expected_exn -> (
             ...
             with e -> check exn input_case expected_exn e))
...
let urlenc_known () =
  ...
  let decode_only_cases =
    [ ("kxc\xaa.~", "kxc%aa.~"); ("kxc.dev/kxclib", "kxc.dev%2fkxclib") ]
  in
  bidirectional_cases
  |> List.iter (fun (raw, encoded) ->
         check string
           (sprintf "urlenc_encode: %S" raw)
           encoded
           (Url_encoding.encode (Bytes.of_string raw));
         check string
           (sprintf "urlenc_decode: %S" raw)
           raw
           (Url_encoding.decode encoded |> Bytes.to_string));
  decode_only_cases
  |> List.iter (fun (raw, encoded) ->
         check string
           (sprintf "urlenc_decode: %S" raw)
...
let rec pp_jv ppf : jv -> unit = function
  ...
  | `obj xs ->
      fprintf ppf "`obj %a"
        (List.pp (fun ppf (k, v) -> fprintf ppf "(%S, %a)" k pp_jv v))
        xs

let rec equal_jv (jv1 : jv) (jv2 : jv) =
  match (jv1, jv2) with
  | `null, `null -> true
  ...
  | `str s1, `str s2 -> s1 = s2
  | `arr a1, `arr a2 ->
      List.for_all2 equal_jv (List.sort compare a1) (List.sort compare a2)
  | `obj o1, `obj o2 ->
      List.for_all2
        (fun (k1, v1) (k2, v2) -> k1 = k2 && equal_jv v1 v2)
        (List.sort compare o1) (List.sort compare o2)
  | _, _ -> false
...
let json_of_jsonm_arr_0 = json_of_jsonm [ `As; `Ae ] (`arr [])
let json_of_jsonm_arr_1 = json_of_jsonm [ `As; `Null; `Ae ] (`arr [ `null ])

let json_of_jsonm_arr_2 =
  json_of_jsonm [ `As; `Null; `Bool true; `Ae ] (`arr [ `null; `bool true ])
...
let json_of_jsonm_obj_0 = json_of_jsonm [ `Os; `Oe ] (`obj [])

let json_of_jsonm_obj_1 =
  json_of_jsonm [ `Os; `Name "a"; `Null; `Oe ] (`obj [ ("a", `null) ])
...
let json_of_jsonm_obj_4 =
  json_of_jsonm
    ...
    (`obj
      [ ("a", `null); ("b", `bool false); ("c", `num 0.); ("d", `str "hello") ])
...
let jvpath_access = jvpath_access' (Jv.access, "Json.access", jv)

let jvpath_access_cases =
  [
    test_case "Json.access_0_1" `Quick (jvpath_access [] `null (some `null));
    test_case "Json.access_0_2" `Quick
      (jvpath_access [] (`str "hello") (some (`str "hello")));
    test_case "Json.access_0_3" `Quick
      (jvpath_access []
         (`arr [ `num 0.; `str "abc" ])
         (some (`arr [ `num 0.; `str "abc" ])));
    ...
    test_case "Json.access_1_1" `Quick
      (jvpath_access
         ...
         (`obj [ ("f1", `num 0.); ("f2", `str "abc") ])
         none);
    test_case "Json.access_1_2" `Quick (jvpath_access [ `i 0 ] (`arr []) none);
    test_case "Json.access_1_3" `Quick
      (jvpath_access [ `i 1 ] (`arr [ `bool true ]) none);
    test_case "Json.access_2_1" `Quick
      (jvpath_access [ `i 0 ] (`arr [ `num 0.; `str "abc" ]) (some (`num 0.)));
    ...
    test_case "Json.access_int_1" `Quick
      (jvpath_access'
         (Jv.access_int, "Jv.access_int", int)
         [ `i 1 ]
         (`arr [ `null; `num 1. ])
         (some 1));
    ...
    test_case "Json.access_arr_2'" `Quick
      (jvpath_access'
         (Jv.access_arr' (Jv.access_int [ `f "age" ]), "Jv.access_arr", list int)
         []
         (`arr
           [
             `obj [ ("name", `str "alice"); ("age", `num 12.) ];
         ...
         ( Jv.access_arr'
             Jv.(
               fun person ->
                 ...
                 access_str [ `f "name" ] person >>? fun name -> some (name, age)),
           "Jv.access_arr",
...
let json_escaped_suite =
  ...
  let case orig escaped =
    ...
    test_case (sprintf "json_escaped_%d" id) `Quick (fun () ->
        check string
          (sprintf "json_escape: %s => %s" (String.escaped orig) escaped)
...
let json_unparse =
  ...
  let case jv unparsed =
    ...
    test_case (sprintf "json_unparse_%d" id) `Quick (fun () ->
        check string
          (sprintf "json_unparsed: %s" unparsed)
  ...
  [
    ( "json_unparse",
      [
        ...
        case
          (`obj
             [ ("_1_num", `num 1.); ("_2_arr", `arr [ `null; `str "hello" ]) ]
          |> Json.normalize)
  ...
  [
    ( "json_show",
      [
        ...
        case (`str "\196\163\n\230\151\165cca\xF0\x93\x83\x93")
...
let jvpath_unparse =
  ...
  let case jvpath unparsed id =
    test_case (sprintf "jvpath_unparse_%d: %s" id unparsed) `Quick (fun () ->
        check string
          (sprintf "jvpath_unparse_check: %s" unparsed)
          unparsed
...
let jv_pump_fields =
  ...
  let pos_case fns j m id =
    ...
    test_case (sprintf "jv_pump_fields_pos_%d" id) `Quick (fun () ->
        ...
        check bool
          (sprintf "pumped pass matcher test; pumped: %a" pp_jv pumped)
          true (m pumped))
  in
    ...
    ( "jv_pump_fields",
      [
        ...
        pos_case [ "a"; "b" ]
          (`obj [ ("a", `num 1.); ("b", `num 2.); ("c", `num 3.) ])
          (function
            | `obj (("a", `num 1.) :: ("b", `num 2.) :: _) -> true | _ -> false);
        pos_case [ "c"; "b" ]
          (`obj [ ("a", `num 1.); ("b", `num 2.); ("c", `num 3.) ])
          (function
            | `obj (("c", `num 3.) :: ("b", `num 2.) :: _) -> true | _ -> false);
        neg_case [] (`num 0.);
        neg_case [ "d" ]
          (`obj [ ("a", `num 1.); ("b", `num 2.); ("c", `num 3.) ]);
        neg_case [ "a"; "d" ]
...
let () =
  ...
  run "Kxclib_baselib_unit_tests"
    ([
       ("trivial", [ test_case "trivial_case" `Quick trivial ]);
       ("seq_iteri", [ test_case "seq_iteri_0" `Quick seq_iteri_0 ]);
       ("seq_range", [ test_case "seq_range_0" `Quick seq_range_0 ]);
       ( "seq_make",
         [
    ...
    @ stream_suite @ string_partition @ json_escaped_suite @ json_unparse
    @ json_show @ jvpath_unparse @ jvpath_parse_success @ jv_pump_fields
    @ [
        ( "base64",
          [
        ...
        ("url_encoding", [ test_case "urlenc_known" `Quick urlenc_known ]);
        ( "json_of_jsonm",
          [
