...
(** [reverse f] reverses the order of arguments of [f]. *)
let reverse_arg f x y = f y x

(** [refupdate' f r] is equivalent to [refupdate r f]. *)
let refupdate' x r = (reverse_arg refupdate) x r

(** [refappend' x r] is equivalent to [refappend r x]. *)
let refappend' x r = (reverse_arg refappend) x r
...
let refupdate'_and_get f r =
  let result = f !r in
  r := result;
  result

let get_and_refupdate' f r =
  let x = !r in
  r := f !r;
  x
...
module Functionals = struct
  ...
  let reptill judge f x =
    let rec loop y = if judge y then y else loop (f y) in
    loop (f x)
        ...
        fun f x ->
          let rec loop n (x, x') =
            if x = x' then x
              ...
              failwith' "fixpoint not reached after %d tries" maxn
            else loop (pred n) (x', f x')
          in
          loop (pred maxn) (x, f x)

  let converge' judge f =
    let rec loop n (x, x') =
      match judge n x x' with true -> Ok x' | false -> loop (succ n) (x', f x')
    in
    fun x -> loop 1 (x, f x)

  let converge judge f x = converge' (fun _ x x' -> judge x x') f x

  module BasicInfix = struct
    (** function composition 1 *)
    let ( % ) : ('y -> 'z) -> ('x -> 'y) -> 'x -> 'z = fun f g x -> x |> g |> f
    ...
    let ( &> ) : ('x -> 'y) -> ('y -> 'z) -> 'x -> 'z = fun g f x -> x |> g |> f

    let ( ?. ) : ('a -> 'b -> 'c) -> 'b -> 'a -> 'c = dig2nd
    let ( ?.. ) : ('a -> 'b -> 'c -> 'd) -> 'c -> 'a -> 'b -> 'd = dig3rd
    let ( !. ) : 'b -> ('a -> 'b -> 'c) -> 'b -> 'c = fix2nd
    let ( !.. ) : 'c -> ('a -> 'b -> 'c -> 'd) -> 'a -> 'b -> 'd = fix3rd
    ...
    let ( /> ) : 'a * 'b -> ('b -> 'c) -> 'a * 'c = fun (a, b) f -> (a, f b)
    let ( /< ) : 'a * 'b -> ('a -> 'c) -> 'c * 'b = fun (a, b) f -> (f a, b)

    (** lift to snd *)
    let ( ?> ) : ('b -> 'c) -> 'a * 'b -> 'a * 'c = fun f (a, b) -> (a, f b)

    (** lift to fst *)
    let ( ?< ) : ('a -> 'c) -> 'a * 'b -> 'c * 'b = fun f (a, b) -> (f a, b)

    (** calculate to snd *)
    let ( ?+> ) : ('a -> 'b) -> 'a -> 'a * 'b = fun f a -> (a, f a)

    (** calculate to fst *)
    let ( ?+< ) : ('a -> 'b) -> 'a -> 'b * 'a = fun f a -> (f a, a)
    ...
    (** uncurry *)
    let ( !! ) : ('a -> 'b -> 'x) -> 'a * 'b -> 'x = fun f (a, b) -> f a b
...
struct
  ...
  (** piping iter *)
  let ( |!> ) : 'x t -> ('x -> unit) -> unit = fun xs f -> iter f xs
...
module MonadOps (M : sig
  ...
  val return : 'x -> 'x t
  val bind : 'x t -> ('x -> 'y t) -> 'y t
end) : MonadOpsS with type 'x t := 'x M.t = struct
  let return x = M.return x
  let ( >>= ) = M.bind
  let ( >> ) : 'x M.t -> 'y M.t -> 'y M.t = fun ma mb -> ma >>= fun _ -> mb
...
module ResultOf (E : sig
  type err
end) : ResultOfS with type err = E.err = struct
  type err = E.err
...
module ResultWithErrmsg0 = struct
  include ResultOf' (struct
    type err = string

    let string_of_err = Option.some
  end)

  let protect' : handler:(exn -> string) -> ('x -> 'y) -> 'x -> 'y t =
   fun ~handler f x -> try Ok (f x) with exn -> Error (handler exn)
...
module Option0 = struct
  ...
  let or_raise e = function Some x -> x | None -> raise e
  ...
  let pp vpp ppf =
    Format.(
      function
      | Some x -> fprintf ppf "Some(%a)" vpp x | None -> fprintf ppf "None")
  ...
  let protect : ?capture:(exn -> bool) -> ('x -> 'y) -> 'x -> 'y option =
   fun ?(capture = constant true) f x ->
    try f x |> some with exn -> if capture exn then none else raise exn
...
module Seq0 = struct
  ...
  let from : (unit -> 'x option) -> 'x t =
   fun f ->
    let rec next () =
      match f () with None -> Nil | Some x -> Cons (x, next)
    in
    next
  ...
  let iota until_exclusive =
    ...
    from (fun () ->
        let x = !counter in
        if x = until_exclusive then None
        else (
          incr counter;
          Some x))
  ...
  let iotaf' f n = iota n |!> f
  let length s = fold_left (fun c _ -> succ c) 0 s
  ...
  let make n x =
    ...
    | _ when n < 0 -> failwith "panic"
    | _ ->
        let rec h i () = match i with 0 -> Nil | _ -> Cons (x, h (i - 1)) in
        h n
end
...
module Array0 = struct
  ...
  let concat_map f arr = arr |> to_seq |> Seq.flat_map (f &> to_seq) |> of_seq
      ...
      let rec labor left right =
        ...
        | rlen ->
            if rlen < 0 then (0., 0)
              ...
              let lv, lw = labor left mid and rv, rw = labor (succ mid) right in
              let ( ! ) = float_of_int in
              (((lv *. !lw) +. (rv *. !rw)) /. !(lw + rw), lw + rw)
      in
      labor 0 (len - 1) |> fst
  ...
  let first arr = match length arr with 0 -> raise Not_found | _ -> arr.(0)
  let last arr = match length arr with 0 -> raise Not_found | n -> arr.(n - 1)
  ...
  let update : ('a -> 'a) -> 'a array -> int -> unit =
   fun f arr idx -> arr.(idx) <- f arr.(idx)

  let update_each : (int -> 'a -> 'a) -> 'a array -> unit =
   fun f arr -> arr |> Array.iteri (fun i x -> arr.(i) <- f i x)
  ...
  let blastsati : ('a -> bool) -> 'a array -> int =
   fun pred arr ->
    ...
    let rec loop pred l r =
      ...
      else if l + 1 = r then if pred r then r else l
      else if l = r then if l = 0 && not (pred l) then raise Not_found else l
      else
        let m = (l + r) / 2 in
...
/\  forall k >  i, (pred arr.(k)) = false v}
  ...
  let blastsat : ('a -> bool) -> 'a array -> 'a =
   fun pred arr -> blastsati pred arr |> Array.get arr
  ...
  let shuffle : ?rng:(int (* bound_exclusive *) -> int) -> 'x array -> unit =
   fun ?(rng = Random.int) arr ->
    let len = Array.length arr in
    for i = len - 1 downto 1 do
      swap arr i (rng (succ i))
    done

  let to_function : 'a array -> int -> 'a = fun arr idx -> arr.(idx)
end
...
module Stream = struct
  ...
  let hd stream =
    let open Stream in
    try next stream with Failure -> raise Not_found

  let drop1 stream =
    let open Stream in
    let _ = try next stream with Failure -> raise Not_found in
    stream
  ...
  let take n stream =
    ...
    in
    match List.length m_lst with m when m = n -> m_lst | _ -> raise Not_found
...
module List0 = struct
  ...
  module Ops_piping = PipeOps (struct
    include List
  ...
  let iota1 = function 0 -> [] | k -> List.init k succ
   ...
   fun k func l ->
    let l', updated =
      ...
      |> fold_left
           ...
           ([], false)
    in
    if not updated then match func none with None -> l | Some v -> (k, v) :: l
    else rev l'
  ...
  let deassoc_opt' : 'k -> ('k * 'v) list -> ('v * ('k * 'v) list) option =
   fun k es ->
    match deassoc_opt k es with Some v, es -> Some (v, es) | None, _ -> None

  (** same as [deassq_opt] but different return type *)
  let deassq_opt' : 'k -> ('k * 'v) list -> ('v * ('k * 'v) list) option =
   fun k es ->
    match deassq_opt k es with Some v, es -> Some (v, es) | None, _ -> None
   ...
   fun kf l ->
    ...
    |> fold_left
         (fun acc x ->
           let k = kf x in
           update_assoc k
             (function Some xs -> x :: xs |> some | None -> some [ x ])
             acc)
         []
  ...
  let reduce_opt f = function [] -> none | hd :: tl -> foldl f hd tl |> some
  ...
  let last list = foldl (fun _ x -> x) (List.hd list) list

  (** last element and rest of a list *)
  let and_last : 'x. 'x list -> 'x list * 'x =
   fun xs -> match rev xs with [] -> raise Not_found | l :: r -> (rev r, l)
  ...
  let pp ?sep ?parens vpp ppf xs =
    let open Format in
    let popen, pclose =
      match parens with Some parens -> parens | None -> ("[", "]")
    in
...
module Hashtbl = struct
  ...
  let rev : ('a, 'b) t -> ('b, 'a) t =
   fun orig -> to_seq orig |> Seq.map (fun (k, v) -> (v, k)) |> of_seq
...
module String = struct
  ...
  let partition_opt n str =
    ...
    if n > len then None else Some (sub str 0 n, sub str n (len - n))
  ...
  let partition n str =
    ...
    |> Option.v' (fun () ->
           invalid_arg'
             "Kxclib.String.partition - n (= %d) is greater than the length \
              (=%d) of %S"
             n (length str) str)
   ...
   fun str ->
    ...
    let rec check_bytek = function
      ...
      | pos, remaining ->
          ...
          && check_bytek (succ pos, pred remaining)
    in
    let rec check pos =
      if pos = len then true
        ...
        if c land 0x80 = 0 then check (succ pos) (* 1byte utf8 *)
          ...
          check_bytek (succ pos, 1) (* 2byte utf8 *) && check (pos + 2)
        else if c land 0xf0 = 0xe0 then
          check_bytek (succ pos, 2) (* 3byte utf8 *) && check (pos + 3)
        else if c land 0xf8 = 0xf0 then
          check_bytek (succ pos, 3) (* 4byte utf8 *) && check (pos + 4)
        else false (* invalid *)
    in
    check 0
  ...
  let valid_ascii_js_identifier s =
    ...
    let rec loop n =
      if n >= len then true
        ...
        match at n with
        | '_' | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' -> loop (succ n)
        | _ -> false
    in
    if len = 0 then false
    else match at 0 with '_' | 'a' .. 'z' | 'A' .. 'Z' -> loop 1 | _ -> false
  ...
  let pp_json_escaped : Format.formatter -> string -> unit =
   fun ppf str ->
    ...
    let addu x =
      adds "\\u";
      adds (Format.sprintf "%04x" x)
    in
    let addb n k = iotaf' (fun i -> addc (getc (n + i))) k in
    let flush = Format.pp_print_flush ppf in
    let raise' pos =
      invalid_arg' "json_escaped: invalid/incomplete utf-8 string at pos %d" pos
    in
    let rec loop n =
      if (n - 1) mod 64 = 0 then flush ();
      let adv k = loop (n + k) in
      let check k = if not (n + k <= len) then raise' (n + k - 1) in
      if succ n > len then ()
      else
        match getc n with
        | '"' ->
        ...
        | c ->
            ...
            if x1 < 32 then (
              ...
              let spit k =
                ...
                adv k
              in
              if x1 land 0xe0 = 0xc0 then spit 2 (* 2byte utf8 *)
              else if x1 land 0xf0 = 0xe0 then spit 3 (* 3byte utf8 *)
              else if x1 land 0xf8 = 0xf0 then spit 4 (* 4byte utf8 *)
              else raise' n)
    in
    loop 0

  let json_escaped : string -> string = Format.asprintf "%a" pp_json_escaped
end
...
module MapPlus (M : Map.S) = struct
  let pp' kpp vpp ppf m =
    ...
    |> List.iter'
         (fun (key, value) ->
           fprintf ppf "@[<hov 0>%a=@,@[<hov 0>%a@];@]@;<1 2>@?" kpp key vpp
             value)
         (fun (key, value) ->
           fprintf ppf "@[<hov 0>%a=@,@[<hov 0>%a@];@] }@?" kpp key vpp value);
    pp_close_box ppf ()
...
module IoPervasives = struct
  ...
  let slurp_input ?buf ic =
    let buf =
      match buf with None -> Bytes.make 4096 '\000' | Some buf -> buf
    in
    let result = ref "" in
    let rec loop len =
  ...
  let spit_file path str = with_output_file path (Fn.flip output_string str)
end
...
module Timing = struct
  ...
  let timefunc f =
    let time = ref 0. in
    timefunc' time f |> ignore;
    !time
end

module Int53p = struct
  type int53p_impl_flavor =
    [ `int_impl | `int64_impl | `float_impl | `custom_impl of string ]

  let pp_int53p_impl_flavor : Format.formatter -> int53p_impl_flavor -> unit =
   fun ppf ->
    let open Format in
    function
  ...
  let show_int53p_impl_flavor : int53p_impl_flavor -> string =
    Format.asprintf "%a" pp_int53p_impl_flavor
  ...
  module Internals = struct
    ...
    module FloatImpl : S = struct
      ...
      let to_int, of_int =
        ...
        | Other "js_of_ocaml" | Other "BS" -> (Obj.magic, Obj.magic)
        | _ -> Float.(to_int, of_int)

      let round_towards_zero x =
        let open Float in
        if x < 0. then x |> neg % floor % neg else floor x
    ...
    let impl_of_builtin_flavor : int53p_impl_flavor -> (module S) = function
      ...
      | flavor ->
          failwith' "non-builtin int53p_impl_flavor: %a" pp_int53p_impl_flavor
            flavor
...
module Datetime0 : sig
  ...
  let daycount_of_month ~leap =
    let table = if leap then days_of_months_leap else days_of_months_nonleap in
    fun mm -> table mm
  ...
  let day_of_year yy =
    let table =
      ...
      | true -> days_of_months_subsum_leap
    in
  ...
  struct
    module Conf = struct
      ...
      let max_year =
        let span =
          pred Int.max_int / (366 * 24 * 60 * 60 * subsecond_resolution)
        in
        span - 1 + min_year
    end
    ...
    let yearcount_leaping ymin ymax =
      let roundup div x = if x mod div = 0 then x else div * succ (x / div) in
      let ncat div =
        let span = ymax - roundup div ymin in
        if span < 0 then 0 else succ (span / div)
      in
      let ncat4 = ncat 4 in
    ...
    let normalize ?subsec ?tzoffset (yy, mm, dd) (hour, min, sec) =
      ...
      if yy < min_year || yy > max_year then
        invalid_arg
          Format.(
            asprintf
              "%s.normalize - timestamp cannot be handled: %d-%d-%d \
               %02d:%02d:%02d (subsec: %d/%d) - year out of range (%d-%d)"
              "/kxclib.ml/.Datetime0.EpochNormalizedTimestamp" yy mm dd hour min
              sec subsec subsecond_resolution min_year max_year);
      ...
      let days_past_years =
        ...
        (leaping * 366) + (nonleaping * 365)
      in
      let doy = day_of_year yy mm dd in
      let hour, min =
        match tzoffset with
        | None -> (hour, min)
        | Some (tzhour, tzmin) -> (hour + tzhour, min + tzmin)
      in
      let nts =
        sec + (min * 60)
        + (hour * 60 * 60)
        + ((days_past_years + doy) * 24 * 60 * 60)
      in
      let nts = (nts * subsecond_resolution) + subsec in
      nts
  end

  module UnixTimestmapSecRes = EpochNormalizedTimestamp (struct
    let epoch_year = 1970
    let subsecond_resolution = 1
  end)

  module UnixTimestmapMilliRes = EpochNormalizedTimestamp (struct
    let epoch_year = 1970
    let subsecond_resolution = 1000
  end)

  module UnixTimestmapNanoRes = EpochNormalizedTimestamp (struct
    let epoch_year = 1970
...
module ParseArgs = struct
  ...
  let scanfparser fmt fn : optparser =
   fun str ->
  ...
  let parse_opts (optparsers : optparser list) ?(argsource = (Sys.argv, 1)) () =
    let rec tryparse str = function
      ...
      | p :: ps -> (
          ...
          | `Process_next false -> ())
    in
    Array.to_list (fst argsource)
    |> List.drop (snd argsource)
  ...
  let parse_opts_args ?(optprefix = "-") ?(optsep = "--")
    ...
    let prefixed str =
      ...
      else String.sub str 0 optprefixlen = optprefix
    in
    ...
    let rec loop n parseopt =
      if n >= argc then List.rev !args
        ...
        let arg = source.(n) in
        if not parseopt then (
          refappend args arg;
          loop (succ n) parseopt)
        else if arg = optsep then loop (succ n) false
        else if prefixed arg then (
          tryparse arg;
          loop (succ n) parseopt)
        else (
          refappend args arg;
...
module ArgOptions = struct
  ...
  module type FeatureRequests = sig
    ...
    val get_args :
      ?argsource:string array * int -> ?optsep:string -> unit -> string list
  end
  ...
  let get_option ?argsource ?(prefix = "") ?optsep (type x) :
    ...
    let labor opt f =
      ...
      match !state with
      | `Init -> !result
      | `CaptureNext -> invalid_arg ("no argument supplied to option " ^ opt)
    in
    function
    | IntOption opt -> labor opt (fun arg -> Scanf.sscanf arg "%i%!" identity)
    | FloatOption opt -> labor opt (fun arg -> Scanf.sscanf arg "%g%!" identity)
  ...
  let get_option_exn ?argsource ?prefix ?optsep (type x) : x named_option -> x =
   fun opt ->
    match get_option ?argsource ?prefix ?optsep opt with
    | None ->
        invalid_arg ("you have to provide option " ^ opt_of_named_option opt)
    | Some x -> x

  let get_option_d' ?argsource ?prefix ?optsep (type x) :
      x named_option -> (unit -> x) -> x =
   fun opt vp ->
    match get_option ?argsource ?prefix ?optsep opt with
    | None -> vp ()
    | Some x -> x
...
module FmtPervasives = struct
    ...
    let colored ?style ?color_mode:(m = `Fg) color ppf fmt =
      if !color_enabled then (
        ...
        let style_code =
          style |> function
          | None -> None
          | Some s -> style_table s |> sprintf "[%dm" |> Option.some
        in
  ...
  let pp_of_to_string to_string ppf x = Format.pp_print_string ppf (to_string x)
  ...
  let pp_ref_address ppf (r : 'x ref) = fprintf ppf "%#x" (2 * Obj.magic r)
  ...
  let pp_integer_sep' ~padding ppf x =
    let rec loop acc x =
      if x > 0 then loop ((x mod 1000) :: acc) (x / 1000) else acc
    in
    ...
    | Some (d, pad) ->
        ...
        if d > slen then Fn.ntimes (d - slen) (fun () -> pp_char ppf pad) ());
    pp_string ppf str
  ...
  let pp_exn ppf exn = Printexc.to_string exn |> Format.pp_print_string ppf
...
module Log0 = struct
  ...
  let log ~label ?modul ?header_style:(style = None)
    ...
    let header = match modul with None -> label | Some m -> label ^ ":" ^ m in
    let header =
      match !timestamp_func () with
      | None -> sprintf "[%s]" header
      | Some ts -> sprintf "[%s :%.3f]" header ts
    in
  ...
  let error ?modul fmt =
    log ?modul fmt ~label:"ERROR" ~header_style:(Some `Bold) ~header_color:`Red
...
type backtrace_info =
  [ `ocaml_backtrace of Printexc.raw_backtrace | `string_stacktrace of string ]
...
module Direct_io = struct
  ...
  let inject_error' : exn * backtrace_info option -> 'x t =
   fun (exn, bt) ->
    let bt =
      bt
      |> Option.v' (fun () -> `ocaml_backtrace (Printexc.get_raw_backtrace ()))
    in
    Result.error (exn, bt)
...
module Json : sig
  ...
  type jv_kind = [ `null | `bool | `num | `str | `arr | `obj ]
  ...
  val parse_jvpath' :
    ?check_root:
      [ `check_root_prefix
        (** default, check the prefix dot denoting the document root *)
      | `no_check  (** do not care whether there is a prefix dot or not *) ] ->
    string ->
    (jvpath, [ `pos of int | `premature_end of int ]) result
  ...
  let sort_by_key fs =
    fs |> List.sort_uniq (fun (k1, _) (k2, _) -> String.compare k1 k2)
  ...
  let rec eqv a b =
    ...
    | `obj fs1, `obj fs2 ->
        ...
        let rec loop = function
          | [], [] -> true
          | (k1, x) :: xs, (k2, y) :: ys when k1 = k2 && eqv x y -> loop (xs, ys)
          | _ -> false
        in
        loop (fs1, fs2)
    | _ -> false
  ...
  and normalize_fields : jv_fields -> jv_fields =
   fun fs -> sort_by_key fs |&> fun (k, v) -> (k, normalize v)

  let rec pp_unparse ppf =
    ...
    | `obj fs ->
        ...
        |> List.iter'
             (fun (f, j) ->
               outf "\"%a\":%!%a,%!" String.pp_json_escaped f self j)
             (fun (f, j) ->
               outf "\"%a\":%!%a}%!" String.pp_json_escaped f self j)
  ...
  let rec pp_lit ppf =
    ...
    | `arr xs ->
        ...
        xs
        |> List.iter'
             (fun value -> fprintf ppf "@[<hov 0>%a@];@;<1 2>@?" self value)
             (fun value -> fprintf ppf "@[<hov 0>%a@]]@?" self value)
    | `obj [] -> outs "`obj []"
    | `obj fs ->
        outf "`obj [";
        fs
        |> List.iter'
             (fun (key, value) ->
               fprintf ppf "@[<hov 0>%S, @,@[<hov 0>%a@];@]@;<1 2>@?" key self
                 value)
             (fun (key, value) ->
               fprintf ppf "@[<hov 0>%S, @,@[<hov 0>%a@]@]]@?" key self value)
  ...
  let reserved_javascript_keywords =
    StringSet.of_list
      [
        "break";
        "case";
  ...
  let pp_jvpath : ppf -> jvpath -> unit =
   fun ppf ->
    ...
    let rec go : [ `root of bool ] * jvpath -> unit = function
      | `root true, [] -> outs "."
      | `root false, [] -> ()
      | `root is_root, `f fname :: rest -> continue_fname is_root fname rest
    ...
    and continue_fname forced_dot fname rest =
      ...
      if needs_escape then (
        ...
        go (`root false, rest))
    in
    fun path -> go (`root true, path)

  let unparse_jvpath : jvpath -> string = sprintf "%a" pp_jvpath

  module ParseJvpathResult = ResultOf' (struct
  ...
  let parse_jvpath' :
    ...
    fun ?(check_root = `check_root_prefix) input ->
      ...
      let sub start end_ =
        StringLabels.sub input ~pos:start ~len:(end_ - start)
      in
      let ch pos =
        if pos >= len then error (`premature_end pos)
        else String.get input pos |> ok
      in
      let next ~n pos =
        if n + pos > len then error (`premature_end pos)
        else ok (n + pos, sub pos (pos + n))
      in
      let rec advance_whitespaces expect_non_whitespace pos =
        if (not expect_non_whitespace) && pos = len then ok pos
        else
          ch pos >>= function
          | ' ' | '\t' | '\r' | '\n' | '\x0C' (* form feed *) ->
      ...
      let expect_ch c pos =
        if pos = len then error (`premature_end pos)
          ...
          ch pos >>= fun c' ->
          if c' = c then ok (succ pos) else error (`pos pos)
      in
      let expect_prefix prefix pos =
        let plen = String.length prefix in
        if plen + pos > len then error (`premature_end pos)
      ...
      let expect_prefix' prefix pos =
        ...
        expect_prefix prefix pos
      in
      let rec continue_identifier pos0 pos =
        if pos = len then ok (pos, sub pos0 pos)
        else
          ch pos >>= function
          | '0' .. '9' | '_' | 'a' .. 'z' | 'A' .. 'Z' ->
      ...
      let rec continue_numeric pos0 pos =
        if pos = len then ok (pos, sub pos0 pos)
          ...
          ch pos >>= function
          | '0' .. '9' -> continue_numeric pos0 (succ pos)
          | _ -> ok (pos, sub pos0 pos)
      in
      let rec continue_quoted ?(buf = Buffer.create 8) pos =
        ...
        if pos = len then error (`premature_end pos)
          ...
          | '\\' -> (
              ...
              | 'u' -> (
                  next ~n:4 pos'
                  >|= ?>(Option.protect (fun s ->
                             Scanf.sscanf s "%04x%!" identity))
                  >>= function
                  | pos, Some ucode ->
                      addu ucode;
                      continue_quoted ~buf pos
      ...
      let rec go ctx acc pos =
        ...
        if pos = len then ok acc
          ...
          | (`neutral | `root), '[' -> go `within_bracket acc (succ pos)
          | `neutral, '.' -> go `expect_fname acc (succ pos)
          | `within_bracket, '0' .. '9' ->
              let pos0 = pos in
              continue_numeric pos (succ pos) >>= fun (pos, idx) ->
              int_of_string_opt idx |> Option.to_result ~none:(`pos pos0)
              >>= fun idx ->
              expect_ch ']' pos >>= fun pos -> go `neutral (`i idx :: acc) pos
          | `within_bracket, '"' ->
              continue_quoted (succ pos) >>= fun (pos, fname) ->
              expect_ch ']' pos >>= fun pos -> go `neutral (`f fname :: acc) pos
          | (`expect_fname | `root), _ -> err
          | _ -> err
      in
      let go0 pos = go `root [] pos >|= List.rev in
      match (check_root, len) with
      | `check_root_prefix, _ -> expect_prefix' "." 0 >>= go0
      | `no_check, 0 -> ok []
      | `no_check, _ -> (
          advance_whitespaces false 0 >>= fun pos ->
          if pos = len then ok []
          else ch pos >>= function '.' -> go0 (succ pos) | _ -> go0 pos)

  let parse_jvpath_opt ?check_root =
    parse_jvpath' ?check_root &> Result.to_option

  let parse_jvpath_exn ?check_root =
    parse_jvpath' ?check_root &> ParseJvpathResult.get
  ...
  let rec of_yojson : yojson -> jv = function
    | `Null -> `null
    | `Bool x -> `bool x
  ...
  let rec to_yojson : jv -> yojson = function
    | `null -> `Null
    | `bool x -> `Bool x
    | `num x ->
        if
  ...
  let of_jsonm' : 'loc jsonm' -> (jv * 'loc jsonm', 'loc jsonm_pe) result =
   fun input ->
    ...
    let with_next (sloc : 'loc) (next : 'loc jsonm')
        (kont : 'loc -> 'loc jsonm' -> jsonm_token -> 'r) : 'r =
      match next () with
      | Seq.Nil -> Error (`premature_end sloc)
      | Seq.Cons ((nloc, ntok), next') -> kont nloc next' ntok
    in
    let ok next x : (jv * 'loc jsonm', 'loc jsonm_pe) result = Ok (x, next) in
    let rec value loc next = function
      ...
      | (`Name _ | `Ae | `Oe) as tok -> Error (`unexpected_token_at (loc, tok))
      | _ -> .
    and collect_array acc sloc next = function
      ...
      | #value_starting_jsonm_token as head ->
          ...
          with_next sloc next (fun _nloc -> collect_array (v :: acc) sloc)
  ...
  let rec to_jsonm : jv -> jsonm = function
    ...
    | `arr xs ->
        Seq.cons `As
          (List.fold_right
             (fun x seq -> Seq.append (to_jsonm x) seq)
             xs (Seq.return `Ae))
    | `obj xs ->
        Seq.cons `Os
          (List.fold_right
...
module Jv : sig
  ...
  let pump_fields fnames : jv -> jv = function
    ...
    | `obj fs as jv ->
        ...
        let hit, miss =
          ...
          |@> ( (SMap.empty, []),
                fun ((hit, miss), (fn, fv)) ->
                  if SSet.mem fn fnset then (SMap.add fn fv hit, miss)
                  else (hit, (fn, fv) :: miss) )
        in
        if SMap.cardinal hit = len then
          `obj
            (List.rev_append
               (fnames |> List.rev_map (fun fn -> (fn, SMap.find fn hit)))
               miss)
        else jv
    | jv -> jv

  let access : jvpath -> jv -> jv option =
   fun path jv ->
    let rec go = function
      | [], x -> some x
      | `f fname :: path', `obj fs ->
          ...
          |> List.find_map (fun (k, v) ->
                 if k = fname then go (path', v) else none)
      | `i idx :: path', `arr xs ->
  ...
  let access' : (jv -> 'a option) -> jvpath -> jv -> 'a option =
   fun f path jv -> access path jv >>? f

  let access_null : jvpath -> jv -> unit option =
    access' (function `null -> some () | _ -> none)

  let access_bool : jvpath -> jv -> bool option =
    access' (function `bool x -> some x | _ -> none)

  let access_num : jvpath -> jv -> float option =
    access' (function `num x -> some x | _ -> none)
  ...
  let access_int53p : jvpath -> jv -> int53p option =
   fun path jv -> access_num path jv >? Int53p.of_float

  let access_str : jvpath -> jv -> string option =
    access' (function `str x -> some x | _ -> none)

  let access_arr : jvpath -> jv -> jv list option =
    access' (function `arr xs -> some xs | _ -> none)

  let access_obj : jvpath -> jv -> jv_fields option =
    access' (function `obj fs -> some fs | _ -> none)

  let access_arr' : (jv -> 'a option) -> jvpath -> jv -> 'a list option =
   fun f path jv ->
    let open Option.Ops_monad in
    access_arr path jv >? (List.map f &> sequence_list) |> Option.join
end
...
module Base64 = struct
  ...
  module type T = sig
    val encode_buf :
      ?offset:int -> ?len:int -> Buffer.t -> bytes -> int (* written bytes*)
    (**
    ...
    *)
    ...
    val decode_buf :
      ?offset:int -> ?len:int -> Buffer.t -> string -> int (* written bytes *)
    (**
  ...
  module Make (C : Config) : T = struct
    ...
    let encode_buf ?(offset = 0) ?len (output : Buffer.t) (input : bytes) =
      let input_offset, input_end, input_length =
        ...
        if len < 0 || end_index > orig_len then
          invalid_arg'
            "Base64.encode: the input range (offset:%d, len:%d) is out of \
             bounds"
            offset len
      ...
      let output_buf =
        let estimated_chars =
          (input_length / 3 * 4) + 4 (* worst case: (4/3)n + 2 + "==" *)
        in
      ...
      let write i o len =
        ...
        | `S n -> (
            ...
            | `S `I ->
                let b3 = get (i + 2) in
                o
                |> set ((b2 lsl 2) lor (b3 lsr 6)) (* b2[4]..b2[7] b3[0] b3[1]*)
                |> set b3 (* b3[2]..b3[7] *))
      in
      let rec go i o =
        ...
        | 0 -> (
            ...
            | Some pad ->
                ...
                |> List.fold_left
                     (fun o _ ->
                       Buffer.add_char output_buf pad;
                       o + 1)
                     o
            | None -> o)
    ...
    let count_lenth_ignoring ?(offset = 0) ?len (input : Bytes.t) : int =
      ...
      | false, true ->
          let count acc i =
            let b = Bytes.get_uint8 input (offset + i) in
            if is_sixbit b then acc + 1
            else match char_of_int b with '\r' | '\n' -> acc | _ -> acc + 1
          in
          len |> iotafl count 0
      | false, false -> len
    ...
    let decode_buf ?(offset = 0) ?len (output : Buffer.t) (input : string) =
      ...
      let read stack o =
        let set o value =
          Buffer.add_uint8 output_buf (value land 0xff);
          o + 1
        in
        ...
        | s1 :: s2 :: ss -> (
            ...
            | s3 :: ss -> (
                ...
                | [] ->
                    ...
                    if not (s3 land 0x3 = 0) then
                      invalid_arg "Base64.decode: unterminated input"
                    else o
                | s4 :: [] ->
                    (* [3n bytes] *)
                    set o ((s3 lsl 6) lor s4)
                    (* s3[4] s3[5] s4[0]..[5] *)
                | _ -> failwith "impossible"))
      in
              ...
              | c when pad = some c && validate_padding -> (
                  ...
                  let validate_pad ignore =
                      ...
                      | s ->
                          raise
                            (Invalid_base64_padding
                               (`invalid_char_with_position (s, ci + i + 1)))
                    in
                    let pad_num =
                      input_end - i - 1 |> iotafl pad_count 0 |> ( + ) 1
                    in
                    ...
                    if is_valid then read stack o
                      ...
                      raise
                        (Invalid_base64_padding
                           (`invalid_padding_length pad_num))
                  in
                  match (ignore_unknown, ignore_newline) with
                  | true, _ -> read stack o
                  | false, true ->
                      validate_pad (function '\r' | '\n' -> true | _ -> false)
                  | false, false -> validate_pad (fun _ -> false))
              | c ->
                  invalid_arg' "Base64.decode: invalid char '%c' at index %d" c
                    i)
      in
      let total_bytes = go [] input_offset 0 in
      Buffer.add_buffer output output_buf;
...
module Url_encoding : sig
  val encode_buf :
    ?offset:int -> ?len:int -> Buffer.t -> bytes -> int (* bytes written *)
  (**
     Takes an input [bytes], and writes the encoded string to [Buffer.t].
     @param offset   the offset of input which the encoder should start reading from.
     @param len      the length of input which the encoder should read.
  ...
  val decode_buf :
    ?offset:int -> ?len:int -> Buffer.t -> string -> int (* bytes written *)
  (**
     Takes an input [string], and writes the decoded bytes to [Buffer.t].
     @param offset   the offset of input which the decoder should start reading from.
     @param len      the length of input which the decoder should read.
  ...
  let () =
    Printexc.register_printer (function
      | Invalid_url_encoded_string
          ...
          sprintf
            "Invalid_url_encoded_string\n\
            \                      (reserved character %C at %d)" c p
          |> some
      | Invalid_url_encoded_string (`invalid_percent_encoded_character (s, p))
        ->
          sprintf
            "Invalid_url_encoded_string\n\
            \                      (invalid percent encoding %S at %d)" s p
          |> some
      | Invalid_url_encoded_string (`percent_encoded_character_end_of_input p)
        ->
          sprintf
            "Invalid_url_encoded_string('%%' too close to the end-of-input at \
             %d)"
            p
          |> some
      | _ -> none)
  ...
  let encoding_table = lazy (iotaf (sprintf "%%%02X") 256 |> List.to_function)

  let decoding_table =
    lazy
      (let table1 = Lazy.force encoding_table in
       let table2 = Seq.iotaf ?+<(sprintf "%%%02x") 256 in
       iotaf ?+<table1 256 |> List.to_seq |> Seq.append table2
       |> StringMap.of_seq)
  ...
  let encode_buf ?(offset = 0) ?len buf data =
    ...
    if olen < 0 || offset + olen > dlen then
      invalid_arg'
        "Url_encoding.encode: the input range (offset:%d, len:%d) is out of \
         bounds"
        offset olen;
  ...
  let decode_buf ?(offset = 0) ?len buf data =
    let lookup =
      ...
      fun s -> StringMap.find_opt s table
    in
    let dlen = String.length data in
    let ilen = len |? dlen - offset in
    if ilen < 0 || offset + ilen > dlen then
      invalid_arg'
        "Url_encoding.decode: the input range (offset:%d, len:%d) is out of \
         bounds"
        offset ilen;
    let rec loop p = function
      | 0 -> ilen
