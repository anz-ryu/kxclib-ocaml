...
module Json_ext : sig
  ...
  let _to_jstr : 'x -> any =
   fun v ->
    if _is_null v then jstr "null"
    else if _is_undefined v then jstr "undefined"
    else jstr @@ _call v "toString" [||]
  ...
  let classify : xjv -> xjv_type =
    ...
    fun x ->
      ...
      if typ == _string then `_string
        ...
        else if _call _js_array "isArray" [| x |] |> Js.to_bool then `_array
        else `_object
      else invalid_arg' "unable to classify object %s" (_to_jstr x |> ocstr)
  ...
  let rec to_xjv : jv -> xjv = function
    ...
    | `arr arr ->
        ...
        let v =
          Js.Unsafe.new_obj _js_array [| Array.length arr |> _cast |] |> _cast
        in
        arr |> Array.iteri (fun idx x -> Js.array_set v idx (to_xjv x));
        v |> _cast
    | `obj kvs ->
        Js.Unsafe.obj [||] |-> fun o ->
        kvs |!> fun (key, value) -> Js.Unsafe.set o (jstr key) (to_xjv value)
  ...
  let rec of_xjv : xjv -> jv =
   fun x ->
    ...
    | `_object ->
        ...
        `obj (loop [] (pred len))
    | _ ->
        invalid_arg' "of_xjv: unable to convert non-JSON encodable value %s"
          (_to_jstr x |> ocstr)

  let to_json_string = to_xjv &> Json.output &> Js.to_string

  let of_json_string_opt str =
    try _call _js_json "parse" [| Js.string str |> _cast |] |> of_xjv |> some
    with Js_error.Exn _ -> None
end
