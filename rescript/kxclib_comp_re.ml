...
module Kxclib_comp_re = struct
  module BaseCompInternals = struct
    ...
    let _stringify : 'a. 'a -> string =
     fun x -> [%raw "function(x){return ''+x;}"] (_cast x)
  end

  open BaseCompInternals
  ...
  module Seq = struct
    ...
    let rec filter f seq () =
      ...
      | Nil -> Nil
      | Cons (x, next) ->
          if f x then Cons (x, filter f next) else filter f next ()
  ...
  module Array = struct
    ...
    module Imported = struct
      external _fill_1 : 'a array -> 'a -> 'a array = "fill" [@@bs.send]

      external _fill_3 :
        'a array -> 'a -> (* start *) int -> (* end *) int -> 'a array = "fill"
        [@@bs.send]
    ...
    module Internals = struct
      ...
      let unsafe_fill : 'a. 'a array -> int -> int -> 'a -> unit =
       fun arr ofs len v -> _fill_3 arr v ofs (ofs + len) |> ignore
    ...
    let fill a ofs len v =
      if ofs < 0 || len < 0 || ofs > length a - len then
    ...
    let for_all2 p l1 l2 =
      let n1 = length l1 and n2 = length l2 in
    ...
    let sort cmp a =
      let maxson l i =
        ...
        if i31 + 2 < l then (
          ...
          !x)
        else if i31 + 1 < l && cmp (get a i31) (get a (i31 + 1)) < 0 then
          i31 + 1
  ...
  module List = struct
    ...
    let of_seq seq =
      let rec direct depth seq : _ list =
        if depth = 0 then
          Seq.fold_left (fun acc x -> x :: acc) [] seq |> rev (* tailrec *)
        else
          match seq () with
          | Seq.Nil -> []
  ...
  module Hashtbl = struct
    ...
    let to_seq tbl =
      ...
      fold (fun k v acc -> (k, v) :: acc) tbl [] |> List.to_seq
  ...
  module Bytes = struct
    ...
    let get_uint8 buf i = get buf i |> int_of_char
  ...
  module String = struct
    ...
    let exists f s = B.exists f (bos s)
    let for_all f s = B.for_all f (bos s)
  end
  ...
  module Buffer = struct
    ...
    let sub b ofs len =
      if ofs < 0 || len < 0 || ofs > b.position - len then
    ...
    let nth b ofs =
      if ofs < 0 || ofs >= b.position then invalid_arg "Buffer.nth"
    ...
    *)
    ...
    let truncate b len =
      if len < 0 || len > length b then invalid_arg "Buffer.truncate"
      else b.position <- len
  ...
  module Json_ext = struct
    ...
    let rec to_xjv : jv -> xjv = function
      ...
      | `obj fs ->
          let o = _obj_make () in
          List.iter (fun (k, v) -> _obj_set o k (to_xjv v)) fs;
          o |> _cast

    let rec of_xjv : xjv -> jv =
     fun x ->
      ...
      | JSString s -> `str s
      | JSFunction f ->
          invalid_arg ("of_xjv: function not expected: " ^ _stringify f)
      | JSObject obj ->
          if Js.Array.isArray obj then
            ...
            `obj fs
      | JSSymbol symb ->
          invalid_arg ("of_xjv: function not expected: " ^ _stringify symb)

    let json_of_xjv : xjv -> string = fun xjv -> Js.Json.stringify (xjv |> _cast)
    let json_of_jv : jv -> string = fun jv -> jv |> to_xjv |> json_of_xjv

    let xjv_of_json_exn : string -> xjv =
     fun json -> Js.Json.parseExn json |> _cast

    let xjv_of_json : string -> xjv option =
     fun json -> try xjv_of_json_exn json |> Option.some with _ -> None

    let jv_of_json_exn : string -> jv =
     fun json -> json |> xjv_of_json_exn |> of_xjv

    let jv_of_json : string -> jv option =
     fun json -> json |> xjv_of_json |> Option.map of_xjv
    ...
    module TestSamples = struct
      ...
      let obj01 : jv =
        (`obj [ ("name", `str "James Smith"); ("age", `num 12.) ] : jv)
  ...
  module JsInterop = struct
    ...
    let bytes_of_uint8array : Js.TypedArray2.Uint8Array.t -> Bytes.t =
     fun arr ->
    ...
    let uint8array_of_bytes : Bytes.t -> Js.TypedArray2.Uint8Array.t =
     fun bytes ->
